%load('./../corrected3/revised80002.mat');
%close all;

%load('./../corrected3/revised90015.mat');

%load('./../corrected4/from_os_prior_20000.mat');
%load('./../corrected4/v14-4.mat');
load('./v23.mat');
load('./../../data/triplicate_data.mat');
color_of_the_fit = [250 218 221]/255;

%%
transient_id = 30000;
%chain = repmat(theta_start,mcmcoptions.nsimu,1) + chain.*repmat(theta_std,mcmcoptions.nsimu,1);


close all;



transparency = 0.1;
color = [70/255,130/255,180/255];

figure(1)
subplot(2,5,1)
errorbar(time/60,mean(1e3*host1'),std(1e3*host1'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,2)
errorbar(time/60,mean(1e3*host2'),std(1e3*host2'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,3)
errorbar(time/60,mean(1e3*host3'),std(1e3*host3'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,4)
errorbar(time/60,mean(1e3*host4'),std(1e3*host4'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,5)
errorbar(time/60,mean(1e3*host5'),std(1e3*host5'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');


subplot(2,5,6)
errorbar(time/60,mean(1e3*virus1'),std(1e3*virus1'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,7)
errorbar(time/60,mean(1e3*virus2'),std(1e3*virus2'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,8)
errorbar(time/60,mean(1e3*virus3'),std(1e3*virus3'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,9)
errorbar(time/60,mean(1e3*virus4'),std(1e3*virus4'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');
subplot(2,5,10)
errorbar(time/60,mean(1e3*virus5'),std(1e3*virus5'),'o','MarkerSize',8, 'MarkerEdgeColor','k','MarkerFaceColor',[70/255,130/255,180/255],Color = [70/255,130/255,180/255]);hold on;
set(gca, 'YScale', 'log');





    cutoff = 250;


for i = transient_id:5000:50000
    i

    %error_store(i) = loglikefun(chain(i,:),data,pars2,mcmcpars,model,0);
if loglikefun(chain(i,:),data,pars2,mcmcpars,model,0) < cutoff

 pars_samples = update_pars(pars2,chain(i,:),mcmcpars);
 pars_samples.beta2 = pars_samples.beta;

[t3,S3,V3,~] = simulate_ode(model,pars_samples,tvec,pars2.S0,pars2.V0);
    figure(1)
    subplot(2,5,6)
    patchline(t3,V3(:,1),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;
  
    subplot(2,5,7)
    patchline(t3,V3(:,2),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

    subplot(2,5,8)
    patchline(t3,V3(:,3),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

    subplot(2,5,9)
    patchline(t3,V3(:,4),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

    subplot(2,5,10)
    patchline(t3,V3(:,5),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

    
    subplot(2,5,1)
    patchline(t3,S3(:,1),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

    subplot(2,5,2)
    patchline(t3,S3(:,2),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;
    
    subplot(2,5,3)
    patchline(t3,S3(:,3),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;
    
    subplot(2,5,4)
    patchline(t3,S3(:,4),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;
    
    subplot(2,5,5)
    patchline(t3,S3(:,5),'edgecolor',color_of_the_fit,'linewidth',2,'edgealpha',transparency);hold on;

end

end

%% touch up

figure(1)

subplot(2,5,1)
hold on;
set(gca,'FontSize',18);
title('CBA 4');
ylabel({'Host density','(cells/ml)'})
xticks(0:2:16);
ylim([1e4 1e9]);
yticks([1e4 1e5, 1e6, 1e7, 1e8 1e9]);
xlim([0 16]);
axis square;

subplot(2,5,2)
hold on;
set(gca,'FontSize',18);
title('CBA 18');
xticks(0:2:16);
ylim([1e4 1e9]);
yticks([1e4 1e5, 1e6, 1e7, 1e8 1e9]);
xlim([0 16]);
axis square;

subplot(2,5,3)
hold on;
set(gca,'FontSize',18);
title('CBA 38');
xticks(0:2:16);
ylim([1e4 1e9]);
yticks([1e4 1e5, 1e6, 1e7, 1e8 1e9]);
xlim([0 16]);
axis square;


subplot(2,5,4)
hold on;
set(gca,'FontSize',18);
title('PSA H100');
xticks(0:2:16);
ylim([1e4 1e9]);
yticks([1e4 1e5, 1e6, 1e7, 1e8 1e9]);
xlim([0 16]);
axis square;

subplot(2,5,5)
hold on;
set(gca,'FontSize',18);
title('PSA 13-15');
xticks(0:2:16);
ylim([1e4 1e9]);
yticks([1e4 1e5, 1e6, 1e7, 1e8 1e9]);
xlim([0 16]);
axis square;


subplot(2,5,6)
hold on;
set(gca,'FontSize',18);
title('\phi18:2');
ylabel({'Phage density','(virions/ml)'})
xticks(0:2:16);
ylim([1e4 1e11]);
yticks([1e4, 1e6, 1e8, 1e10]);
xlim([0 16]);
axis square;



subplot(2,5,7)
hold on;
set(gca,'FontSize',18);
title('\phi18:3');
xticks(0:2:16);
ylim([1e4 1e11]);
yticks([1e4, 1e6, 1e8, 1e10]);
xlim([0 16]);
axis square;


subplot(2,5,8)
hold on;
set(gca,'FontSize',18);
title('\phi38:1');
xticks(0:2:16);
ylim([1e4 1e11]);
yticks([1e4, 1e6, 1e8, 1e10]);
xlabel('Time (hrs)');
xlim([0 16]);
axis square;


subplot(2,5,9)
hold on;
set(gca,'FontSize',18);
title('PSA HP1');
xticks(0:2:16);
ylim([1e4 1e11]);
xlim([0 16]);
yticks([1e4, 1e6, 1e8, 1e10]);
axis square;


subplot(2,5,10)
hold on;
set(gca,'FontSize',18);
title('PSA HS6');
xticks(0:2:16);
ylim([1e4 1e11]);
yticks([1e4, 1e6, 1e8, 1e10]);
xlim([0 16]);
axis square;
    

% figH = gcf;
% set(figH, 'Units', 'centimeters',...
%           'OuterPosition', [0 0 90 20])
% exportgraphics(figH,...
%                'seivd-comm-all.png',...
%                'BackgroundColor', 'none',...
%                'ContentType','vector'); 
