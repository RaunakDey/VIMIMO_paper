...
theta_mu(1:9) = [1.81,118.2,60.5,20,8.246,99.2,437,93,413];
theta_mu(10:18) = log([5.14e-8,1.45e-8,9.57e-8,2.5e-8,1.227e-7,1.56e-7, 6.46e-8,1.31e-7,8.02e-8 ])/log(10);
theta_mu(19:27)  = [1.749,1.89,2.19,1,1.9,1.47,2.14,1.42,1.97]; 
theta_mu(28:32) = [0.19,0.245,0.22,0.28,0.25];
theta_mu(33:37) = log([5e6 6.13e6 12e6  19.3e5 16.0e5])/log(10);


theta_std = [200, 200, 200, 200, 200, 200, 200, 200, 200, 4,4,4,4,4,4,4,4,4, 2,2,2,2,2,2,2,2,2, 0.5,0.5,0.5,0.5,0.5, 0.05,0.05,0.05,0.05,0.05];

%%% in the new coordination system.
theta_start_new = (theta_start  - theta_mu)./theta_std;
...
mcmcparam{1,i}{1,2} = theta_start_new(i); % this is start value 
mcmcparam{1,i}{1,5} = 0;
mcmcparam{1,i}{1,6} = 1;
end
