...
data_mean = data; %storing for future reference
...
% 
% for i=0:27
% data.ydata(3*i+1:3*i+3,1) = host1(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,2) = host2(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,3) = host3(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,4) = host4(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,5) = host5(i+1,:)'*1e3;
% 
% 
% data.ydata(3*i+1:3*i+3,6) = virus1(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,7) = virus2(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,8) = virus3(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,9) = virus4(i+1,:)'*1e3;
% data.ydata(3*i+1:3*i+3,10) = virus5(i+1,:)'*1e3;
% 
% end
% 
% data.xdata = reshape(repmat(data_mean.xdata,1,3)',84,1);

for i = 1:3
data{1,i}.xdata = data_mean.xdata;
data{1,i}.ydata = [host1(:,i),host2(:,i), host3(:,i), host4(:,i), host5(:,i), virus1(:,i),virus2(:,i), virus3(:,i), virus4(:,i), virus5(:,i) ]*1e3;

end

for i = 1:3
data{1,i}.id = data_mean.id;
data{1,i}.units = data_mean.units;
data{1,i}.labels = data_mean.labels;
end
...
%have to use modelfun to run inference (where the package writes ssfun for
%us)

clear mcmcmodel;

mcmcmodel.modelfun   = @simulate_ode; 
...
%% mcmc run
load('./from_os_prior_16.mat','mcmcresults');
...
save('from_os_prior_tripli_10000.mat')
