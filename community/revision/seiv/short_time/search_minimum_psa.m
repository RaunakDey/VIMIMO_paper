function theta_optimized = search_minimum_psa(theta_initial,sd_theta,data,model,pars,mcmcpars,limit,num_steps, step_size)
...
for j = 1:length(theta)-2
    ...
    error1 = error_psa_short(theta1,pars,data,model,mcmcpars,limit);
    error2 = error_psa_short(theta2,pars,data,model,mcmcpars,limit);
    ...
    cost = error_psa_short(theta,pars,data,model,mcmcpars,limit) 
    if min_val < cost
