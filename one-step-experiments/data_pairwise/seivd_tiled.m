...
% Create an 8x2 tiled layout: 8 cases, host | virus
...
for i = 1:length(Dc_array)
    ...
    total_virus_seivd_pairwise{i} = y_series_seivd(end-1,:);
    temp = sum(y_series_seivd(1:end-2,:));
    temp(temp < cutoff_den) = 0;
    host_den_seivd_paiwise{i} = temp;
...
set(gca,'YScale','log');
xlabel('Time (hrs)'); ylabel('Host/ml');
title('\phi18:2 – CBA 18');
set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]);
mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
set(gca,'YScale','log');
xlabel('Time (hrs)'); ylabel('Phage/ml');
xlim([0 16]);
mean_virus = mean(qpcr_virus,2); err = std(qpcr_virus,0,2);
errorbar([0 175 945]/60, mean_virus, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','r','LineWidth',1.5);
ylim([1e4 5e11]); yticks([1e4 1e6 1e8 1e10]); xticks(0:2:16); box on;
...
moi = mean(qpcr_virus(1,:))/mean(qpcr_host(1,:)); moi_list(end+1) = moi;
...
title('\phi18:3 – CBA 4'); set(gca,'FontSize',16,'FontName','Times New Roman');
...
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('\phi18:3 – CBA 18'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('\phi38:1 – CBA 38'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('PSA-HP1 – PSA H100'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('PSA-HP1 – PSA 13-15'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('PSA-HS6 – PSA H100'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err = std(qpcr_host,0,2);
errorbar([0 175 945]/60, mean_host, err, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);
...
%% Case 8: HS6 13-15 (legend included)
...
% Host (left) with handles for legend
nexttile;
hold on;
hLinesHost = gobjects(length(Dc_array),1);
for i = 1:length(Dc_array)
    ls='-'; if i==length(Dc_array), ls='--'; end
    hLinesHost(i) = plot(time_seivd{i}, host_den_seivd_paiwise{i}, 'LineWidth',4,'Color',gray(i,:),'LineStyle',ls);
end
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Host/ml');
title('PSA-HS6 – PSA 13-15'); set(gca,'FontSize',16,'FontName','Times New Roman');
xlim([0 16]); mean_host = mean(qpcr_host,2); err_h = std(qpcr_host,0,2);
hHost = errorbar([0 175 945]/60, mean_host, err_h, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','b','LineWidth',1.5);
xticks(0:2:16); box on; yticks([1e-6 1e-3 1e0 1e3 1e6 1e9]);

% Virus (right) with handles for legend
nexttile;
hold on;
hLinesVirus = gobjects(length(Dc_array),1);
for i = 1:length(Dc_array)
    ls='-'; if i==length(Dc_array), ls='--'; end
    hLinesVirus(i) = plot(time_seivd{i}, total_virus_seivd_pairwise{i}, 'LineWidth',4,'Color',gray(i,:),'LineStyle',ls);
end
set(gca,'YScale','log'); xlabel('Time (hrs)'); ylabel('Phage/ml');
xlim([0 16]); mean_virus = mean(qpcr_virus,2); err_v = std(qpcr_virus,0,2);
hPhage = errorbar([0 175 945]/60, mean_virus, err_v, 's','MarkerSize',8,'MarkerEdgeColor','k','MarkerFaceColor','k','LineStyle','none','Color','r','LineWidth',1.5);
ylim([1e4 1e11]); yticks([1e4 1e6 1e8 1e10]); xticks(0:2:16); box on;

% Legend combining model lines and data markers
...
legHandles = [hModel; hLinesVirus(:); hData; hPhage; hHost];
legLabels = { ...
    '\bf Model simulation', ...
