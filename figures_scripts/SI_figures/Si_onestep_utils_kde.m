...
%%
% Assume 'chain' is [samples x parameters]
burn = 1;  % If needed (not used here)
...
N = size(chain_1, 1);  % Samples per chain
...
plot(xaxis, gaussian_truncated( xaxis,  log( results.prior(2,1)) /log(10),0.4 ,log( params{2,1}{1,3})/log(10),log(params{2,1}{1,4})/log(10)), 'k',LineWidth=2,LineStyle='--');
...
plot(xaxis, gaussian_truncated( xaxis,   results.prior(3,1),results.prior(3,2),params{3,1}{1,3}, params{3,1}{1,4}),'k',LineWidth=2,LineStyle='--');
xlim([xaxis(1) xaxis(end)]);
xlim([0.5 5]);
...
plot(xaxis, gaussian_truncated( xaxis,   results.prior(4,1),results.prior(4,2),params{4,1}{1,3}, params{4,1}{1,4}),'k',LineWidth=2,LineStyle='--');
xlim([0.5 600]);
...
plot(xaxis, gaussian_truncated( xaxis,   results.prior(5,1),results.prior(5,2),params{5,1}{1,3}, params{5,1}{1,4}),'k',LineWidth=2,LineStyle='--');
xlim([xaxis(1) xaxis(end)]);
xlim([5 400]);
