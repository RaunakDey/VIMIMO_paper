...
%load('./../../community/noninformative2.mat');
%chain = chain_continues2;
%chain = repmat(theta_start,length(chain),1) + chain.*repmat(theta_std,length(chain),1);
...
[fp,xfp] = kde(chain(transient_id:end,i));
plot(xfp,fp,LineWidth=2,Color='b');
clear xfp fp;
...
% Data
x = 1:length(R_2);  % X-axis positions
y = R_2;            % Your Gelman-Rubin values

% Color map (same groupings as before)
colors = zeros(length(R_2), 3);

colors(1:9, :)     = repmat([211, 23, 24] ./ 255, 9, 1);        % Red group
colors(10:18, :)   = repmat([15, 104, 82] ./ 255, 9, 1);        % Green group
colors(19:27, :)   = repmat([211, 119, 46] ./ 255, 9, 1);       % Orange group
colors(28:32, :)   = repmat([62, 137, 168] ./ 255, 5, 1);       % Blue group
colors(33:37, :)   = repmat([217, 76, 33] ./ 255, 5, 1);        % Dark orange group

% Plot points
scatter(x, y, 100, colors, 'filled');  % 100 = marker size
...
set(gca,'FontSize',20);
ylabel('R_{GR}')
%yticks([0 0.2 0.4 0.6 0.8 0.9 1 1.1]);
...
scatter(1:length(geweke_stats), geweke_stats, 100, colors, 'filled');  % 100 = marker size
hold on;
